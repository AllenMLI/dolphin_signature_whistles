...
class GradCAM:
    ...
    def find_target_layer(self):
        """
        Attempt to find the final convolutional layer in the network by looping over
    ...
    def compute_heatmap(self, image, eps=1e-8):
        ...
        with tf.GradientTape() as tape:
            ...
            if len(predictions[0]) == 1: # single number output
                loss = predictions[0]
            else: # softmax-type output (vector of num_classes length)
        ...
        # the convolution and guided gradients have a batch dimension (which we don't need) so let's grab
        # the volume itself and discard the batch
        conv_outputs = conv_outputs[0]
        guided_grads = guided_grads[0]

        # compute the average of the gradient values, and using them as weights, compute the ponderation
        # of the filters with respect to the weights
    ...
    def overlay_heatmap_detection(self, heatmap, image, alpha=0.5,
        ...
        output = cv2.addWeighted(image.squeeze(), alpha, heatmap, 1 - alpha, 0)
